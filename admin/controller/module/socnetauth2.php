	...
	public function index() {   
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) 
		{
			...
			if( !empty($this->request->post['stay']) )
			{
				...
				$this->redirect($this->url->link('module/socnetauth2', 'token=' . $this->session->data['token'].'&tab='.$tab, 'SSL'));
			}
			else
			{
		...
		elseif( $this->config->has('socnetauth2_widget_name') )
		{
			...
			$this->data['socnetauth2_widget_name'] = $this->config->get('socnetauth2_widget_name');
		}
		else
		{
		...
		$this->data['entry_mailru_status'] = $this->language->get('entry_gmail_status');
		...
		if (isset($this->request->post['socnetauth2_account_format'])) {
			...
			$this->data['socnetauth2_account_format'] = 'kvadrat';
		}
		...
		if (isset($this->request->post['socnetauth2_checkout_format'])) {
			...
			$this->data['socnetauth2_checkout_format'] = 'kvadrat';
		}
		...
		if (isset($this->request->post['socnetauth2_simple_format'])) {
			...
			$this->data['socnetauth2_simple_format'] = 'bline';
		}
		...
		if (isset($this->request->post['socnetauth2_simplereg_format'])) {
			...
			$this->data['socnetauth2_simplereg_format'] = 'bline';
		}
