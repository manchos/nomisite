<?php
class Modelmerchmerchproduct extends Model {
	...
	public function getProducts($data = array()) {
		if ($data) {
			...
			if (!empty($data['filter_category']) && !is_null($data['filter_category'])) {
			...
			if (!empty($data['filter_name'])) {
				...
				$sql .= " OR LCASE(pd.name) LIKE '%" . $this->db->escape(utf8_strtolower($data['filter_name'])) . "%'";
				$sql .= " OR LCASE(pd.name) LIKE '%" . $this->db->escape(utf8_strtolower($data['filter_name'])) . "')";
			}
			
			if(!empty($data['filter_artikul'])) {
				$sql .= " AND (LCASE(p.sku) LIKE '" . $this->db->escape(utf8_strtolower($data['filter_artikul'])) . "%'";
				$sql .= " OR LCASE(p.sku) LIKE '%" . $this->db->escape(utf8_strtolower($data['filter_artikul'])) . "%'";
				$sql .= " OR LCASE(p.sku) LIKE '%" . $this->db->escape(utf8_strtolower($data['filter_artikul'])) . "')";
			}
