...
Class SocAuth extends DB
{
	...
	public function checkNew($data)
	{
		...
		*/	
			...
			$this->query_run("UPDATE `" . DB_PREFIX . "customer` 
							  ...
							  WHERE
							    customer_id = '" .$this->escape($data['customer_id']) . "'");
	...
	public function get_config_param($param_name, $group='socnetauth2')
	{
		...
		WHERE `group`='".$group."' AND `key`='".$param_name."'");
	...
	public function addCustomer($data)
	{
		...
		$customer_group_id = $this->get_config_param('socnetauth2_'.$data['provider'].'_customer_group_id', 
													 'socnetauth2');
