...
class ControllerModuleCarousel extends Controller {
	protected function index($setting) {
		...
		$this->document->addScript('catalog/view/javascript/jquery/fancybox/jquery.fancybox.pack.js?v=2.1.5');
                $this->document->addStyle('catalog/view/javascript/jquery/fancybox/jquery.fancybox.css?v=2.1.5');       
...
//		}
                ...
                if (file_exists('catalog/view/theme/' . $this->config->get('config_template') . '/stylesheet/owl.carousel.css')) {
			...
			$this->document->addStyle('catalog/view/theme/default/stylesheet/owl.carousel.css');
                ...
                if (file_exists('catalog/view/theme/' . $this->config->get('config_template') . '/stylesheet/owl.theme.css')) {
		...
		foreach ($results as $result) {
			if (file_exists(DIR_IMAGE . $result['image'])) {
				$this->data['banners'][] = array(
					'title' => $result['title'],
					'link'  => $result['link'],
                                        'simage' => $this->model_tool_image->getImageUrl($result['image']),
