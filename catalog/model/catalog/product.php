<?php
    ...
    class ModelCatalogProduct extends Model {
       ...
       public function getProductImagesBack($product_id) {
       ...
       public function getProduct($product_id) {
          ...
          $query = $this->db->query("SELECT DISTINCT *, pd.name AS name, p.image, m.name AS manufacturer, "
                  ...
                  . "AND pd2.customer_group_id = '" . (int) $customer_group_id . "' AND pd2.quantity = '1' AND "
                  . "((pd2.date_start = '0000-00-00' OR pd2.date_start < '" . $this->NOW . "') AND "
                  . "(pd2.date_end = '0000-00-00' OR pd2.date_end > '" . $this->NOW . "')) "
...
//          2014 (c) Izyum Web System www.izyum.name 
          ...
          $imageback = array();
          $imageback = $this->model_catalog_product->getProductImagesBack($product_id);
          if (!isset($imageback['1']['image'])) {
             $imageback['1']['image'] = null;
          }
          $imb = $imageback['1']['image'];
...
public function getProducts($data = array()) {
          ...
          if (!empty($data['filter_category_id'])) {
             if (!empty($data['filter_sub_category'])) {
                ...
                if (!empty($data['coll_id'])){
                    $sql .= " AND p2c.category_id IN ('" . (int) $data['filter_category_id'] ."','".(int) $data['coll_id']."')";
                }
                else{
                    $sql .= " AND p2c.category_id = '" . (int) $data['filter_category_id'] . "'";
                }
          ...
          if (isset($data['start']) || isset($data['limit'])) {
            ...
            // $sql .= " LEFT JOIN " . DB_PREFIX . "product_to_category p2c USING (product_id) WHERE category_id IN (" . (int) $data['filter_category_id'] .",".(int) $data['coll_id'].") GROUP BY 1 HAVING COUNT(*)=2";
          }
...
//		$product_data['sql']=$sql;
       ...
       public function getBestSellerProducts($limit) {
          ...
          if (!$product_data) {
             ...
             foreach ($query->rows as $result) {
                $product_data[$result['product_id']] = $this->getProduct($result['product_id']);
//                $product_data[$result['product_id']]['image2'] = $result['image2'];
             }
       ...
       public function getCategories($product_id) {
          $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_category, " . DB_PREFIX . "category_description WHERE " . DB_PREFIX . "product_to_category.product_id = '" . (int) $product_id . "' AND " . DB_PREFIX . "product_to_category.category.id =" . DB_PREFIX . "category_description.category.id");
